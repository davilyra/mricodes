...
if(flag_reim)
	...
	for i=1:size(input,3)
		if(~isempty(threshold))
			fprintf('using threshold = [%2.2f]...\n',threshold);
			[output_mask_real(:,:,i),output_real(:,:,i)]=etc_polyfit2(input_real(:,:,i),order,'threshold',threshold);
		elseif(~isempty(mask))
			%fprintf('using provided image mask...\n');
			[output_mask_real(:,:,i),output_real(:,:,i)]=etc_polyfit2(input_real(:,:,i),order,'mask',mask);
		else
			[output_mask_real(:,:,i),output_real(:,:,i)]=etc_polyfit2(input_real(:,:,i),order);
		end;
	end;
	...
	for i=1:size(input,3)
		if(~isempty(threshold))
			fprintf('using threshold = [%2.2f]...\n',threshold);
			[output_mask_imag(:,:,i),output_imag(:,:,i)]=etc_polyfit2(input_imag(:,:,i),order,'threshold',threshold);
		elseif(~isempty(mask))
			%fprintf('using provided image mask...\n');
			[output_mask_imag(:,:,i),output_imag(:,:,i)]=etc_polyfit2(input_imag(:,:,i),order,'mask',mask);
		else
			[output_mask_imag(:,:,i),output_imag(:,:,i)]=etc_polyfit2(input_imag(:,:,i),order);
		end;
	end;
