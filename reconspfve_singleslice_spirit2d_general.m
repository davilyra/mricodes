...
useinversion = 1; % [0] no, [1] yes - this follows what is presented in Lyra-Leite et al. EMBC 34:416, 2012.
...
%==========================================================================
        ...
        for p = 1:nphases, %loops through the cardiac cycle
            ...
            if useinversion == 0
                ...
                if ~(usespirit)
                    ...
                    for v = 1:nVE, %loops through the kv levels
                        xyc = spirPI(kxkyckv(:,:,:,v),kxkyweights,kxkytraj,[pixels,pixels],kSize,us);
        ...
        if (savedata2disk) && ncoils == 4
            ...
            cd recondata/spirit_599
            if ~(usespirit)
                if ~(useinversion)
                    save(sprintf('recon_sos_no_inversion_slice_%d_us_%d.mat',slice,undersamplingfactor), 'xyvt', 'xy', 'maxveloc', 'optr');
                else
                    save(sprintf('recon_sos_inversion_slice_%d_us_%d.mat',slice,undersamplingfactor), 'xyvt', 'xy', 'maxveloc', 'optr');
                end
            else
                if ~(useinversion)
                    save(sprintf('recon_spirit_no_inversion_slice_%d_us_%d.mat',slice,undersamplingfactor), 'xyvt', 'xy', 'maxveloc', 'optr');
                else
                    save(sprintf('recon_spirit_inversion_slice_%d_us_%d.mat',slice,undersamplingfactor), 'xyvt', 'xy', 'maxveloc', 'optr');
                end
            end
            disp('Done!')
            disp(' ')
