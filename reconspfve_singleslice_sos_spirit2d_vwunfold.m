...
for s = 1:nslices,
    ...
    for us = 1:length(accelfactors),
        ...
        if(usespirit), % SPIRIT CALIBRATION
            ...
            disp(' '),fprintf('starting NUFFT+SPIRiT+SOS reconstruction...'),disp(' ')
        else
            disp(' '),fprintf('starting NUFFT+SOS reconstruction...'),disp(' ')
        end;
            ...
            if usespirit,
                ...
                if useparfor,
                    ...
                    for v = 1:nVE,
                        ...
                        xyvc(:,:,v,:) = permute(xyc,[1 2 4 3]);
                        fprintf('p =%3d/%d, v =%3d/%d',p,nphases,v,nVE) % display current cardiac phase
                        disp(' ')
                    end
                end
                toc;
        ...
        if(reconpsf) %displays point-spread-function  
            ...
            if(savedata2disk)
                if ~(usespirit)
                    ...
                    save(sprintf('datapaper/spirit_JL1/recon_slice_sos_slice_%d_us_%d_method_%d.mat',slice,undersamplingfactor,viewordering), 'xyvt', 'xy', 'maxveloc', 'optr');
                    disp('Done!')
                else
                    disp('saving reconstructed data to disk...')
